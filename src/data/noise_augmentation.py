...
class EnvironmentalNoiseAugmentation(nn.Module):
    ...
    def forward(self, waveforms):
        ...
        fade_len = min(1000, waveforms.shape[-1] // 20)
        fade = torch.linspace(0, 1, fade_len, device=device)
        noise[..., :fade_len] *= fade
        noise[..., -fade_len:] *= fade.flip(0)
        ...
        target_snr = torch.empty(
            (batch_size,) + (1,) * (waveforms.ndim - 1), device=device
        ).uniform_(self.min_snr_db, self.max_snr_db)
